...
#if defined(NDEBUG)
# error "Arnix cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfde6da201b83ad329c356eb35c8aaad08460520be08e10be6b407ece5733b288");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Arnix: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Arnix Signed Message:\n";
...
{
    // Arnix: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Arnix: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Arnix: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Arnix: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Arnix: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Arnix: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfde6da201b83ad329c356eb35c8aaad08460520be08e10be6b407ece5733b288");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Arnix";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x44822e069beccac3ff111e1b231ca7aa625338240f64ccfd5140f4a8406e315a"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Arnix: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArnixMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArnixMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ArnixMiner(CWallet *pwallet)
{
    printf("ArnixMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Arnix-miner");
    ...
    try { loop {
        ...
        printf("Running ArnixMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ArnixMiner, pwallet));
}
